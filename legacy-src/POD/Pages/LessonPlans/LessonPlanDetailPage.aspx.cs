    ...
    public partial class LessonPlanDetailPage : System.Web.UI.Page
    {
        ...
        private void LoadLessonPlan()
        {
            ...
            else
            {
                ...
                this.hfStartDate.Value = set.StartDate.ToString();
                this.hfEndDate.Value = set.EndDate.ToString();
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            ...
            if (set != null)
            {
             ...
             var lessonPlanStartDate = Convert.ToDateTime(set.StartDate).ToString("yyyy/MM/dd");
            var lessonPlanEndDate = Convert.ToDateTime(set.EndDate).ToString("yyyy/MM/dd");
            rvlpdates.MinimumValue = lessonPlanStartDate;
            rvlpdates.MaximumValue = lessonPlanEndDate;
            }
        ...
        protected void ButtonPrint_Click(object sender, EventArgs e)
        {
            ...
            GeneratePDFdata(LessonPlanSetID, LessonPlanID );
           
        }

        protected void ButtonPrintAll_Click(object sender, EventArgs e)
        {

            var lessonPlanList = POD.Logic.ProgramCourseClassLogic.GetLessonPlansBySetID(Convert.ToInt32(LessonPlanSetID));

            foreach (var lp in lessonPlanList)
            {
                GeneratePDFdata(Convert.ToInt32(LessonPlanSetID), lp.LessonPlanID);
            }
            //GeneratePDFdata(LessonPlanSetID, LessonPlanID);
        ...
        private readonly Regex _cleanupComments2 = new Regex(@"<!--\[.*?\]-->", RegexOptions.Compiled);
